...
public class TPoseHandler extends AbstractPreCopyAnglesAnimationHandler {
    ...
    public void onRender() {
        if (!get(Minecraft.getMinecraft().thePlayer.getUniqueID()).isAnimating() && get(Minecraft.getMinecraft().thePlayer.getUniqueID()).shouldReset()) state = 0.0f;
    }

    @Override
    public float modifyState() {
        return HyperiumGui.clamp(HyperiumGui.easeOut(this.state, 100.0f, 0.01f, 5), 0.0f, 100.0f);
    }
